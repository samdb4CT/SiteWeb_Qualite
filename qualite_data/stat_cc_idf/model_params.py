...
MODEL_CONFIG_PARAMS = {
    'Requests': {
        ...
        'Request_employment' : "SELECT 'nb_employments'::varchar  as nom, sum((jobs.caracteristics->>'value')::int) as count, \'{0}\'::varchar as cc_name from ( select object_id, jsonb_array_elements(data->'number_of_jobs') as caracteristics from {0}.city_object_extensions where city_object_extension_type = 'employment' AND object_id NOT LIKE 'TERRITOR%' ) as jobs" .format(MODEL_BDD_PARAMS['city_context_name']),
        'Request_count_household' : "SELECT 'nb_households'::varchar as nom, count(object_id), \'{0}\'::varchar as cc_name from {0}.city_object_extensions where city_object_extension_type = 'household_abstract_persons'".format(MODEL_BDD_PARAMS['city_context_name'])
    },
    'Requests_params' : {
