...
def launch_UpdData(request):
    ...
    print( "household Request done in %s seconds!" %(seven_request_time - six_request_time))
    res = ("household Request done in %s seconds!" %(seven_request_time - six_request_time))
    #target_connection commit
    target_connection.commit()
    print("commit ok. All data loaded successfully")
    return render(request, 'accueil.html', {'res' : res})