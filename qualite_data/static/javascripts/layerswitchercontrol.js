...
ol.control.LayerSwitcher.prototype.switchLayerVisibility = function(l, layers)
{
	if (!l.get('baseLayer')){
        ...
        if (l.getVisible()){
            ...
            var legendImg = $('#sidebar').append('<img id=img_'+l.get('id')+' src="'+url + 'REQUEST=GetLegendGraphic&SERVICE=WMS&layer=' + l.getSource().getParams().layers + '&format=image/png"></img>');
        }
        else{
             $('#img_'+l.get('id')+'').remove();
        }
    }
	else 
